        ...
        public void handleDebugEvents(DebugEvent[] events) {
            for (DebugEvent debugEvent : events) {
                if (debugEvent.getSource() instanceof PooslThread) {
                    if (debugEvent.getKind() == DebugEvent.MODEL_SPECIFIC && debugEvent.getDetail() == PooslConstants.INSPECT_RECEIVED) {
                    ...
                    try {
                        if (debugEvent.getKind() == DebugEvent.RESUME || debugEvent.getKind() == DebugEvent.TERMINATE) {
                            ...
                            if (secondaryId != null) {
                                ...
                                if (source instanceof PooslDebugTarget && ((PooslDebugTarget) source).getName().equals(debugTargetName)
                                        && (debugEvent.getKind() == DebugEvent.SUSPEND || debugEvent.getKind() == DebugEvent.CHANGE)) {
