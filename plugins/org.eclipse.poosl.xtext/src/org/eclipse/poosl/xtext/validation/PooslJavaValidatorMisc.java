...
public class PooslJavaValidatorMisc extends PooslJavaValidatorUnusedElements {
        ...
        for (Import imp : poosl.getImports()) {
            ...
            if (importURI != null && !ImportingHelper.isValidImport(imp)) {
                ...
                if (!ImportingHelper.isValidImportLib(imp)) {
                    ...
                    error(MessageFormat.format(INVALID_IMPORT, importURI), imp, null, PooslIssueCodes.INVALID_IMPORT_VALID_IMPORTLIB, importURI);
                }
            } else {
        ...
        for (Import imp : poosl.getImportLibs()) {
            ...
            if (importURI != null && !ImportingHelper.isValidImportLib(imp)) {
                if (!ImportingHelper.isValidImport(imp)) {
                    ...
                    error(MessageFormat.format(INVALID_IMPORTLIB, importURI), imp, null, PooslIssueCodes.INVALID_IMPORTLIB_VALID_IMPORT, importURI);
                }
            } else {
                checkImportLibLocations(imp);
            }
    ...
    /**
     * Checks if importlib locations in the imports are resolved to the same file location as from the current resource
     * location.
     * 
     * @param imp
     */
            ...
            for (Entry<Pair<String, String>, URI> importLibs : importlibPairs.entrySet()) {
                ...
                if (resolved == null) {
                    ...
                    if (originalResolved.isEmpty()) {
                        errorMessage = MessageFormat.format(INVALID_IMPORTLIB_UNRESOLVED_ORIGINAL_AND_LOCAL, originalFile, originalRaw);
                    } else {
                        errorMessage = MessageFormat.format(INVALID_IMPORTLIB_UNRESOLVED_LOCAL, originalFile, originalRaw, originalResolved);
                    }
                    ...
                    if (!originalResolved.isEmpty()) {
                        ...
                        if (!localResolved.equals(originalResolved)) {
                            String originalFile = sourceFileOfImportLib.trimFragment().toString();
                            warning(MessageFormat.format(INVALID_IMPORTLIB_RESOLVED_DIFF, originalFile, originalRaw, originalResolved, localResolved), imp, Literals.IMPORT__IMPORT_URI, null,
                                    WarningType.IMPORT);
                        }
                    }
                }
    ...
    public void checkChannelConnectsMultipleInstances(Channel channel) {
        if (channel.getExternalPort() == null && channel.getInstancePorts().size() > 1) {
            ...
            if (instances.size() == 1) {
                warning(CHANNEL_CONNECTS_ONLY_ONE_INSTANCE, channel, null, PooslIssueCodes.UNCONNECTED_CHANNEL, WarningType.UNCONNECTED);
            }
        }
    }
    ...
    private void warnLocationOfReturnRecursive(Expression expr, boolean isLast, boolean returnHasAlreadyOccurred) {
        if (expr instanceof ReturnExpression) {
            ...
            if (returnHasAlreadyOccurred) {
                warning(RETURN_NESTED_IN_RETURN, expr, null, PooslIssueCodes.LOCATION_OF_RETURN_SUPERFLUOUS, WarningType.RETURN);
            } else if (!isLast) {
                warning(RETURN_NOT_AT_END, expr, null, PooslIssueCodes.LOCATION_OF_RETURN_SUPERFLUOUS, WarningType.RETURN);
            }
            return;
        }
        ...
        if (expr instanceof ExpressionSequence) {
            ...
            if (((SwitchExpression) expr).getDefaultBody() != null) {
                warnLocationOfReturnRecursive(((SwitchExpression) expr).getDefaultBody(), isLast, returnHasAlreadyOccurred);
                return;
            } else {
                if (!isLast) {
                    return;
            ...
            warnLocationOfReturnRecursive(((AssignmentExpression) expr).getExpression(), false, returnHasAlreadyOccurred);
            if (!isLast) {
                return;
            }
        } else if (expr instanceof DataMethodCallExpression) {
            warnLocationOfReturnRecursive(((DataMethodCallExpression) expr).getTarget(), false, returnHasAlreadyOccurred);
            ...
            warnLocationOfReturnRecursive(((BinaryOperatorExpression) expr).getLeftOperand(), false, returnHasAlreadyOccurred);
            warnLocationOfReturnRecursive(((BinaryOperatorExpression) expr).getRightOperand(), false, returnHasAlreadyOccurred);
            if (!isLast) {
                return;
            }
        } else if (expr instanceof UnaryOperatorExpression) {
            warnLocationOfReturnRecursive(((UnaryOperatorExpression) expr).getOperand(), false, returnHasAlreadyOccurred);
            if (!isLast) {
                return;
            }
        } else {
    ...
    private void warnEqualityInsteadOfAssignment(Expression expr, boolean includeLastExpression) {
        if (expr instanceof BinaryOperatorExpression) {
            if (includeLastExpression && ((BinaryOperatorExpression) expr).getName() == OperatorBinary.EQUAL && ((BinaryOperatorExpression) expr).getLeftOperand() instanceof VariableExpression) {
                warning(ASSIGNMENT_SUGGESTION, expr, Literals.BINARY_OPERATOR_EXPRESSION__NAME, PooslIssueCodes.EQUALITY_INSTEAD_OF_ASSIGNMENT, WarningType.ASSIGNMENT);
            } else {
    ...
    public void errorMissingInstanceParameter(Instance instance) {
        ...
        for (String classParameter : classParameterNames) {
            ...
            if (!found) {
                error("Missing instantiation for parameter " + classParameter, instance, Literals.INSTANCE__CLASS_DEFINITION, PooslIssueCodes.MISSING_INSTANCE_PARAMETER);
            }
        }
    }
    ...
    public void warnSendReceiveMismatchUnconnectedPort(Instance instance) {
        ...
        for (String port : ports) {
            if (!isInstancePortUsed(instance, port, aClass)) {
                ...
                List<ConnectedPortsHelper.InstanceAndPort> connectedPorts = ConnectedPortsHelper.getConnectedProcessPorts(instance, port);
    ...
    public void warnSendReceiveMismatchInternalChannel(Channel channel) {
        if (channel.getExternalPort() == null && !channel.getInstancePorts().isEmpty()) {
            ...
            List<ConnectedPortsHelper.InstanceAndPort> connectedPorts = ConnectedPortsHelper.getConnectedProcessPorts(channel);
            warnSendReceiveMismatch(resource, connectedPorts, channel, null, null);
        }
    }

    /*
     * This function is being used by 2 different functions checking for mismatches between sending and receiving
     * messages. The only difference is that one is based on the entrypoint of a channel and the other of an (instance,
     * port). So either channel or (instance, port) in this call will always be null.
     */
    private void warnSendReceiveMismatch(Resource resource, List<ConnectedPortsHelper.InstanceAndPort> connectedPorts, Channel channel, Instance instance, String port) {
        ...
        if (connectedPorts.isEmpty()) {
            if (channel != null) {
                warning("Internal channel hierarchically connects no process ports", channel, null, PooslIssueCodes.UNCONNECTED, WarningType.UNCONNECTED);
            } else {
                warning("Unconnected port '" + port + "' hierarchically connects no process ports", instance, null, PooslIssueCodes.UNCONNECTED, WarningType.UNCONNECTED);
            }
        } else if (connectedPorts.size() == 1) {
            ConnectedPortsHelper.InstanceAndPort pp = connectedPorts.get(0);
            if (channel != null) {
                warning("Internal channel hierarchically connects only process port " + pp.toString(), channel, null, PooslIssueCodes.UNCONNECTED, WarningType.UNCONNECTED);
            } else {
                warning("Unconnected port '" + port + "' hierarchically connects only process port " + pp.toString(), instance, null, PooslIssueCodes.UNCONNECTED, WarningType.UNCONNECTED);
            }
        } else {
            Map<String, Map<ConnectedPortsHelper.InstanceAndPort, List<String>>> sendMap = createMapPortToTypes(resource, connectedPorts, PooslMessageType.SEND);
            Map<String, Map<ConnectedPortsHelper.InstanceAndPort, List<String>>> receiveMap = createMapPortToTypes(resource, connectedPorts, PooslMessageType.RECEIVE);
    ...
    private Map<String, Map<ConnectedPortsHelper.InstanceAndPort, List<String>>> createMapPortToTypes(Resource resource, List<ConnectedPortsHelper.InstanceAndPort> connectedPorts,
        ...
        for (ConnectedPortsHelper.InstanceAndPort portEntry : connectedPorts) {
            if (portEntry.getInstantiableClassName() != null && portEntry.getPortName() != null) {
                Iterable<IEObjectDescription> messageSignatures = Iterables.filter(PooslCache.get(resource).getMessages(portEntry.getInstantiableClassName(), type),
    ...
    private void warnMessageTypeMismatch(Channel channel, Instance instance, String port, Resource resource, Map<String, Map<ConnectedPortsHelper.InstanceAndPort, List<String>>> actualMap,
            Map<String, Map<ConnectedPortsHelper.InstanceAndPort, List<String>>> expectedMap, PooslMessageType expectedMessageType) {
        ...
        for (Entry<String, Map<ConnectedPortsHelper.InstanceAndPort, List<String>>> expectedEntry : expectedMap.entrySet()) {
            ...
            for (Entry<ConnectedPortsHelper.InstanceAndPort, List<String>> expectedPortEntry : expectedPortEntries.entrySet()) {
                ...
                if (filteredActualPortEntries.isEmpty()) {
                    if (channel != null) {
                        warning("On this internal channel, process port " + expectedPortAndProcess.toString() + " can " + expected + " message '" + messageName + "' with " + numberOfParameters
                                + " parameter(s), but no process port can " + actual + " it", channel, null, PooslIssueCodes.UNCONNECTED_INTERNAL_CHANNEL, WarningType.UNCONNECTED);
                    } else {
                        warning("On unconnected port '" + port + "', process port " + expectedPortAndProcess.toString() + " can " + expected + " message '" + messageName + "' with "
                                + numberOfParameters + " parameter(s), but no process port can " + actual + " it", instance, null, PooslIssueCodes.UNCONNECTED_INTERNAL_CHANNEL,
                    ...
                    if (!"".equals(incompatibleMessagesString)) {
                        ...
                        if (channel != null) {
                            warning("On this internal channel, the parameters of message '" + expectedPortAndProcess.toString() + expectedTypeString + messageName + sbExpectedTypes + "' are not a "
                                    + inheritanceType + " of the parameters of the following corresponding messages:" + incompatibleMessagesString, channel, null, null, WarningType.UNCONNECTED);
                        } else {
                            warning("On unconnected port '" + port + "', the parameters of message '" + expectedPortAndProcess.toString() + expectedTypeString + messageName + sbExpectedTypes
                                    + "' are not a " + inheritanceType + " of the parameters of the following corresponding messages:" + incompatibleMessagesString, instance, null, null,
                                    WarningType.UNCONNECTED);
                        }
                    }
                }
    ...
    private static String computeIncompatibleMessages(Resource resource, String actualTypeString, String messageName, Map<ConnectedPortsHelper.InstanceAndPort, List<String>> actualPortEntries,
    ...
    public void errorInitImplementation(NewExpression newExpr) {
        IScope initMethods = HelperFunctions.getGlobalScope(newExpr.eResource(), Literals.DATA_CLASS__DATA_METHODS_NAMED, PooslDataMethodDescription.predicateDataMethodInit(newExpr.getDataClass()));

        if (initMethods.getAllElements().iterator().hasNext()) {
            Iterable<String> initMethodStrings = PooslDataMethodParser.getDataMethodStrings(initMethods.getAllElements());

            EObject container = newExpr.eContainer();
            if (container instanceof DataMethodCallExpression && newExpr == ((DataMethodCallExpression) container).getTarget()) {
                Iterable<IEObjectDescription> initMethodCalled = Iterables.filter(initMethods.getAllElements(),
                        PooslDataMethodDescription.predicateDataMethod(((DataMethodCallExpression) container).getName(), ((DataMethodCallExpression) container).getArguments().size()));
                if (!initMethodCalled.iterator().hasNext()) {
                    // called method isn't an init method
                    error(MessageFormat.format(INIT_CALL_MISSING, newExpr.getDataClass()), newExpr, null, PooslIssueCodes.INIT_CALL_MISSING, Iterables.toArray(initMethodStrings, String.class));
                }
