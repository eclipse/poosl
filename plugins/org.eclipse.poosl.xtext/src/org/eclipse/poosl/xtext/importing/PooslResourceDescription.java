...
public class PooslResourceDescription extends DefaultResourceDescription {
    ...
    protected List<IEObjectDescription> computeExportedObjects() {
        ...
        if (poosl != null) {
            ...
            exportedEObjects.add(
                    EObjectDescription.create("", poosl, PooslDescription.createUserData(poosl))); //$NON-NLS-1$
    ...
    private void computeExportedDataClasses(
            List<IEObjectDescription> exportedEObjects, Poosl poosl) {
        for (DataClass dClass : poosl.getDataClasses()) {
            String className = dClass.getName();
            if (className != null) {
                exportedEObjects.add(EObjectDescription.create(className, dClass,
                ...
                for (DataMethodBinaryOperator dMethod : dClass.getDataMethodsBinaryOperator()) {
                    ...
                    if (methodName != null) {
                        exportedEObjects.add(EObjectDescription.create(methodName.getLiteral(),
                                dMethod,
                                PooslDataMethodDescription.createUserData(className, dMethod)));
                    }
                }
    ...
    private void computeExportedProcessClasses(
            List<IEObjectDescription> exportedEObjects, Poosl poosl) {
        for (ProcessClass pClass : poosl.getProcessClasses()) {
            ...
            if (className != null) {
                ...
                Iterables.addAll(exportedEObjects,
                        computeExportedProcessVariables(pClass, className));
                // compute exported ProcessMethods
                Iterables.addAll(exportedEObjects,
                        computeExportedProcesMethods(pClass.getMethods(), className));
                // compute exported Ports
                Iterables.addAll(exportedEObjects,
                        computeExportedPorts(pClass.getPorts(), className));
    ...
    private void computeExportedClusterClasses(
            List<IEObjectDescription> exportedEObjects, Poosl poosl) {
        for (ClusterClass cClass : poosl.getClusterClasses()) {
            ...
            if (className != null) {
                ...
                // compute exported Variables
                Iterables.addAll(exportedEObjects,
                        computeExportedClusterVariables(cClass, className));
                // compute exported Ports
                Iterables.addAll(exportedEObjects,
                        computeExportedPorts(cClass.getPorts(), className));
            }
        }
    }

    public static void computeExportedDeclarations(
            List<IEObjectDescription> exportedEObjects, List<Declaration> declarations,
            String className, boolean isFromDataClass, boolean isParameter) {
        Iterables.addAll(exportedEObjects, getVariableDescriptionsFromDeclaration(declarations,
                className, isFromDataClass, isParameter));
    }

    public static void computeExportedMessages(
            List<IEObjectDescription> exportedEObjects, List<MessageSignature> messages,
            String className) {
    ...
    public static Iterable<IEObjectDescription> computeExportedPorts(
            Iterable<Port> ports, final String pClassName) {
    ...
    public static Iterable<IEObjectDescription> computeExportedProcesMethods(
            Iterable<ProcessMethod> methods, final String pClassName) {
                    ...
                    public IEObjectDescription apply(ProcessMethod pMethod) {
                        ...
                        if (methodName != null) {
                            return EObjectDescription.create(methodName, pMethod,
                                    PooslProcessMethodDescription.createUserData(pClassName,
                                            pMethod));
                        }
                        return null;
                    }
    ...
    public static Iterable<IEObjectDescription> computeExportedProcessVariables(
            ProcessClass pClass, String pClassName) {
        return Iterables.concat(
                getVariableDescriptionsFromDeclaration(pClass.getParameters(), pClassName, false,
                        true),
                getVariableDescriptionsFromDeclaration(pClass.getInstanceVariables(), pClassName,
                        false, false));
    }

    public static Iterable<IEObjectDescription> computeExportedDataVariables(
            DataClass dClass, String dClassName) {
        return getVariableDescriptionsFromDeclaration(dClass.getInstanceVariables(), dClassName,
                true, false);
    }

    public static Iterable<IEObjectDescription> computeExportedClusterVariables(
            ClusterClass cClass, String cClassName) {
        return getVariableDescriptionsFromDeclaration(cClass.getParameters(), cClassName, false,
                true);
    }

    public static Iterable<IEObjectDescription> getVariableDescriptionsFromDeclaration(
            List<Declaration> declarations, final String className, final boolean isFromDataClass,
            ...
            public Iterable<IEObjectDescription> apply(Declaration declaration) {
                ...
                if (type != null) {
                    return Iterables.transform(declaration.getVariables(),
                            getVariableToDescriptionFunction(type, className, isFromDataClass,
                                    isParameter));
        ...
        Iterable<Iterable<IEObjectDescription>> variableIterators = Iterables.filter(
                Iterables.transform(declarations, declarationToVariableDescriptions),
                Predicates.notNull());
        Iterable<IEObjectDescription> transformed = Iterables.concat(variableIterators);
        return Iterables.filter(transformed, Predicates.notNull());
    }

    private static Function<Variable, IEObjectDescription> getVariableToDescriptionFunction(
            final String type, final String className, final boolean isFromDataClass,
            final boolean isParameter) {
        return new Function<Variable, IEObjectDescription>() {
            @Override
            public IEObjectDescription apply(final Variable variable) {
                final Map<String, String> userData = PooslDeclarationDescription
                        .createUserData(type, className, isFromDataClass, isParameter);
