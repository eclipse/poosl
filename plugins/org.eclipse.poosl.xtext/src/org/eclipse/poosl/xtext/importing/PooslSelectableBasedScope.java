 ...
 * shadow each other if they have the same
 * name. In POOSL, shadowing is typically determined by the combination of name
 * and numbers of parameters.
 * 
 * To allow using the default IQualifiedNameConverter and overloading methods,
 * the SelectableBasedScope never considers
...
public final class PooslSelectableBasedScope extends SelectableBasedScope {
    private PooslSelectableBasedScope(IScope outer, ISelectable selectable,
    ...
    public static IScope createScope(
            IScope outer, ISelectable selectable, Predicate<IEObjectDescription> filter,
