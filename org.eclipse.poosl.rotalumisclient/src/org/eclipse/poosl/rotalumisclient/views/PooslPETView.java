...
public class PooslPETView extends ViewPart implements SelectionListener, KeyListener {
    ...
    IDebugEventSetListener debugEventSetListener = new IDebugEventSetListener() {
        ...
        public void handleDebugEvents(DebugEvent[] events) {
            for (DebugEvent debugEvent : events) {
                ...
                final PooslThread newDebugContext = ViewHelper.getThreadFromEvent(debugContext, debugEvent, getViewSite().getSecondaryId());
                ...
                if (newDebugContext != null) {
                    if (debugEvent.getSource().equals(newDebugContext.getDebugTarget())) {
                        ...
                        if (debugEvent.getKind() == DebugEvent.TERMINATE || (newDebugContext != null && newDebugContext.isTerminated())) {
                            debugContext = null;
                            break;
                        }
                ...
                } else if (debugEvent.getDetail() == PooslConstants.ERROR_STATE || debugEvent.getDetail() == PooslConstants.BREAKPOINT_HIT) {
                    // dont Clear
                } else {
                    removeDebugInstructionPointer(debugEvent.getSource());
    ...
    public void parseTree(PooslThread thread) {
        ...
        for (JAXBElement<? extends TExecutiontreeBase> executionTreeElement : executionTree.getSequentialOrMethodCallOrParallel()) {
        ...
        menu.addMenuListener(new MenuAdapter() {
            ...
            public void menuShown(MenuEvent e) {
                ...
                if (possibleTransitionIds.containsKey(((TExecutiontreeBase) tree.getSelection()[0].getData()).getHandle())) {
    ...
    private void performTransition(TExecutiontreeBase executiontreeBase) {
        ...
        DebugPlugin.getDefault()
                .fireDebugEventSet(new DebugEvent[] { new DebugEvent(new ExecutionTreeContext(debugContext, executiontreeBase), DebugEvent.MODEL_SPECIFIC, PooslConstants.PERFORM_TRANSITION) });
    }
    ...
    private void addChilds(TExecutiontree executionTree, TreeItem treeItem, JAXBElement<? extends TExecutiontreeBase> executionTreeElement, boolean possiblyActive) {
    ...
    private void updateVariables(TExecutiontreeBase treeData) {
        ...
        if (target != null) {
            if (!treeData.getLocal().equals(BigInteger.ZERO)) {
                target.fireEvent(new DebugEvent(new ExecutionTreeContext(debugContext, treeData), DebugEvent.MODEL_SPECIFIC, PooslConstants.INSPECT_REQUEST));
            } else {
    ...
    public void keyPressed(KeyEvent e) {
        if (((e.stateMask & SWT.CTRL) == SWT.CTRL) && e.keyCode == SWT.ARROW_DOWN) {
            ...
            TreeItem[] currentSelection = tree.getSelection();
            if (currentSelection.length > 0 && possibleTransitionIds.containsKey(((TExecutiontreeBase) currentSelection[0].getData()).getHandle())) {
    ...
    private TreeItem getNextPossibleTransition(TreeItem startItem) {
        if (!possibleTransitionIds.isEmpty()) {
            ...
            for (int i = 0; i < treeItems.size(); i++) {
                if (startSearching) {
                    if (possibleTransitionIds.containsKey(((TExecutiontreeBase) treeItems.get(i).getData()).getHandle())) {
    ...
    public Action createProcessStepAction() {
        ...
        try {
            stepIcon = ImageDescriptor.createFromURL(new URL("platform:/plugin/org.eclipse.poosl.rotalumisclient/icons/icon_process_step.png"));
