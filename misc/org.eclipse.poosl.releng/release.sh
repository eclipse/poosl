...
zip -r  ${OUTPUT_DIR}/examples-"${DATE}".zip  org.eclipse.poosl.examples/ org.eclipse.poosl.libraries/ version.txt -x -- *.svn*
...
echo "This distribution of POOSL libraries was created on ${DATE}" > org.eclipse.poosl.libraries/version.txt 
zip -r ${OUTPUT_DIR}/libraries-"${DATE}".zip org.eclipse.poosl.libraries/
cp ${OUTPUT_DIR}/libraries-"${DATE}".zip ${OUTPUT_DIR}/libraries-latest.zip
rm org.eclipse.poosl.libraries/version.txt


# Create a version file and zip it up together with the user contributions. Filters out POOSL settings.
echo "This distribution of POOSL user contributions was created on ${DATE}" > org.eclipse.poosl.user-contributions/version.txt
zip -r ${OUTPUT_DIR}/user-contributions-"${DATE}".zip org.eclipse.poosl.user-contributions/ -x -- *.settings*
cp ${OUTPUT_DIR}/user-contributions-"${DATE}".zip ${OUTPUT_DIR}/user-contributions-latest.zip
rm org.eclipse.poosl.user-contributions/version.txt
