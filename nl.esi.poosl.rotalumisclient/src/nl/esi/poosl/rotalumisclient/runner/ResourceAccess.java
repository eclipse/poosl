...
public class ResourceAccess {
	...
	public static class ArchivedResourceInfo extends ResourceInfo {
		...
		public List<URI> find(Context context, String path, Predicate<String> matcher, String... fileExtensions) {
			String archiveFileName = URI.createURI(location.authority().substring(0, location.authority().length() - 1))
					.toFileString();
			...
			for (URI uri : find(context)) {
				...
				if (new File(parent, fileName).exists())
					return URI.createURI(fileName)
							.resolve(uri.hasTrailingPathSeparator() ? uri : uri.appendSegment(""));
			}
			return null;
		}
	...
	public abstract static class ResourceInfo {
		...
		protected void collect(File root, File file, List<URI> result, Predicate<String> matcher,
		...
		public abstract List<URI> find(Context context, String path, Predicate<String> matcher,
				String... fileExtensions);
